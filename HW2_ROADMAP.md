...
| Print format         | Three floats. Spec wording uses `%f`; reference uses `%.2f`. **[ambiguous]** — accept either if the type is `ARG_FLOAT`. |
...
- [ ] **Shared:** `serial_printf` capture into `g_printfCalls`
- [ ] **Shared:** `stimulus.hpp` (`inject_adc_result`, `press_button`, etc.)
...
- [ ] `check_adc_isr_scaling` — `inject_adc_result(ADCA, 0, 4096)`, `inject_adc_result(ADCA, 1, 2048)`, `inject_adc_result(ADCA, 2, 0)`. Call `ADCA_ISR()`. Assert: `Adca4result == 4096`, `Adca4voltage ≈ 3.0` ±1e-3; `JoystickXresult == 2048`, `JoystickXvoltage ≈ 1.5`; `JoystickYresult == 0`, `JoystickYvoltage ≈ 0.0`. `AdcaInterruptCount` incremented.
...
- [ ] `check_print_cadence` — drive ISR for 1000 ms synthetic; assert ≥ 9 ≤ 11 prints on SCIA (10 ± 10%).
- [ ] `check_print_format` — `expect_format(latest, "<spec'd format>")`, `expect_arg_types(latest, {ARG_FLOAT, ARG_FLOAT, ARG_FLOAT})`. **[ambiguous]**: spec doesn't pin precision; accept any precision in [0,3].
...
- [ ] Print format precision — spec says `%f`, reference uses `%.2f`. Accept any precision; type must be `ARG_FLOAT`.
