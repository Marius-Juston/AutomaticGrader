...
namespace grader {
    namespace {
        ...
        GpioBank gpio_bank_for(const int gpio) {
            if (gpio >= 0 && gpio <= 31) {
        ...
        bool write_gpio_dat_bit(int gpio, const bool high) {
        ...
        volatile ADC_RESULT_REGS *adc_result_regs(const AdcModule m) {
            switch (m) {
        ...
        const char *adc_module_name(const AdcModule m) noexcept {
            switch (m) {
        ...
        volatile uint16_t *adc_result_slot(volatile ADC_RESULT_REGS *regs, const int soc_idx) {
            if (!regs || soc_idx < 0 || soc_idx > 15) return nullptr;
            return &(&regs->ADCRESULT0)[soc_idx];
        }

        volatile SPI_REGS *spi_regs(const SpiModule m) {
            switch (m) {
        ...
        const char *spi_module_name(const SpiModule m) noexcept {
            switch (m) {
        ...
        volatile EQEP_REGS *eqep_regs(const EqepModule m) {
            switch (m) {
        ...
        const char *eqep_module_name(const EqepModule m) noexcept {
            switch (m) {
    ...
    bool button_state(const int gpio_num) {
        return read_gpio_dat_bit(gpio_num);
    }

    void set_gpio_input(int gpio_num, const bool high) {
        spdlog::debug("stimulus: set_gpio_input(GPIO{}, {})", gpio_num, high ? "HIGH" : "LOW");
        write_gpio_dat_bit(gpio_num, high);
    }

    bool read_gpio_input(const int gpio_num) {
        return read_gpio_dat_bit(gpio_num);
    }

    void inject_adc_result(const AdcModule module, int soc_idx, uint16_t value) {
    ...
    uint16_t read_adc_result(const AdcModule module, const int soc_idx) {
        auto *regs = adc_result_regs(module);
        auto *slot = adc_result_slot(regs, soc_idx);
        if (!slot) return 0;
        return *slot;
    ...
    void clear_adc_results(const AdcModule module) {
        auto *regs = adc_result_regs(module);
        if (!regs) return;
        for (int i = 0; i <= 15; ++i) {
    ...
    void inject_spi_rx(const SpiModule module, uint16_t word) {
    ...
    void inject_spi_rx(const SpiModule module, const std::span<const uint16_t> words) {
    ...
    std::size_t spi_rx_fifo_size(const SpiModule module) {
        auto *regs = spi_regs(module);
        if (!regs) return 0;
        return regs->SPIFFRX.bit.RXFFST;
    }

    void clear_spi_state(const SpiModule module) {
        auto *regs = spi_regs(module);
        if (!regs) return;
        regs->SPIRXBUF = 0;
        regs->SPITXBUF = 0;
    ...
    void inject_encoder_count(const EqepModule module, int32_t count) {
    ...
    int32_t read_encoder_count(const EqepModule module) {
        const auto *regs = eqep_regs(module);
        if (!regs) return 0;
        return static_cast<int32_t>(regs->QPOSCNT);
    }

    void inject_lidar_ping(const std::span<const float, LIDAR_FRAME_LEN> distances) {
    ...
    void inject_lidar_pong(const std::span<const float, LIDAR_FRAME_LEN> distances) {
        for (int i = 0; i < LIDAR_FRAME_LEN; ++i) {
    ...
    void inject_lidar_frame(const std::span<const float, LIDAR_FRAME_LEN> distances) {
