...
int check_initialization(Validator *val) {
    ...
    success &= report(EPwm8Regs.TBPRD == 62500,
                      "HW5/init", "EPwm8Regs.TBPRD != 62500 (50 Hz servo carrier)") ? 1 : 0;
    success &= report(EPwm8Regs.TBCTL.bit.CLKDIV == 4,
                      "HW5/init", "EPwm8Regs.TBCTL.CLKDIV != 4") ? 1 : 0;
    success &= report(EPwm8Regs.AQCTLA.bit.ZRO == 2 && EPwm8Regs.AQCTLA.bit.CAU == 1,
                      "HW5/init", "EPwm8Regs.AQCTLA not configured for servo PWM") ? 1 : 0;
    success &= report(EPwm8Regs.AQCTLB.bit.ZRO == 2 && EPwm8Regs.AQCTLB.bit.CBU == 1,
                      "HW5/init", "EPwm8Regs.AQCTLB not configured for servo PWM") ? 1 : 0;

    // ---- EPWM7 SOC trigger for mic ADC (reference: 5000 = 100 us = 10 kHz) ----
    success &= report(EPwm7Regs.TBPRD > 0,
                      "HW5/init", "EPwm7Regs.TBPRD not configured (mic SOC trigger)") ? 1 : 0;
    success &= report(EPwm7Regs.ETSEL.bit.SOCAEN == 1,
                      "HW5/init", "EPwm7Regs.ETSEL.SOCAEN != 1 (mic ADC trigger must be enabled)") ? 1 : 0;
    ...
    // ---- DMA channel 1: source must be one of the ADC result regs ----
    {
        ...
        success &= report(src == adca_addr || src == adcb_addr,
                          ...
                          "DMA source = address of the mic ADC result register") ? 1 : 0;
    }
    ...
    success &= report(DmaRegs.CH1.DST_ADDR_SHADOW != 0,
                      "HW5/init", "DmaRegs.CH1.DST_ADDR_SHADOW not configured") ? 1 : 0;
    ...
    success &= report(DmaRegs.CH1.TRANSFER_SIZE == 1023 || DmaRegs.CH1.TRANSFER_SIZE == 1024,
                      "HW5/init",
                      "DmaRegs.CH1.TRANSFER_SIZE must be 1023 or 1024 (RFFT_SIZE samples)") ? 1 : 0;
    ...
    // Trigger source: 1=ADCAINT1, 6=ADCBINT1 — accept either.
    {
        ...
        success &= report(sel == 1 || sel == 6,
                          "HW5/init",
                          "DmaClaSrcSelRegs.DMACHSRCSEL1.CH1 must be 1 (ADCAINT1) or 6 (ADCBINT1)") ? 1 : 0;
    }
    ...
    success &= report((DmaRegs.CH1.MODE.all & 0x8901) == 0x8901,
                      ...
                      "expected MODE = 0x8901") ? 1 : 0;
    // CONTROL: RUN=1
    success &= report((DmaRegs.CH1.CONTROL.all & 0x0001) == 0x0001,
                      "HW5/init", "DmaRegs.CH1.CONTROL.RUN != 1 (DMA channel must be started)") ? 1 : 0;

    // ---- PIE vectors ----
    if (&DMA_isr) {
        success &= report(PieVectTable.DMA_CH1_INT == &DMA_isr,
                          "HW5/init", "PieVectTable.DMA_CH1_INT != &DMA_isr") ? 1 : 0;
    }
    success &= report(PieCtrlRegs.PIEIER7.bit.INTx1 == 1,
                      "HW5/init", "PieCtrlRegs.PIEIER7.INTx1 != 1 (DMA_CH1 enable)") ? 1 : 0;
    if (&ADCA_isr) {
        success &= report(PieVectTable.ADCA1_INT == &ADCA_isr,
                          "HW5/init", "PieVectTable.ADCA1_INT != &ADCA_isr (joystick ISR)") ? 1 : 0;
    }
...
namespace {
    ...
    int sweep_axis(const char *label, void (*fn)(float), ReadFn read) {
        int success = 1;
        struct Case { float angle; uint16_t expected; };
        const Case cases[] = {
            {  0.0f,  5000},
            {-90.0f,  2500},
            { 90.0f,  7500},
...
int check_print_format(Validator *) {
    ...
    // Reference sets UARTPrint via Timer2 mod 50 (~2 s) — drive enough ticks.
