...
bool consume_flag(char c, uint8_t& flags) noexcept {
    switch (c) {
        case '-': flags |= flag_bits::Minus; return true;
        case '+': flags |= flag_bits::Plus; return true;
        case ' ': flags |= flag_bits::Space; return true;
        case '#': flags |= flag_bits::Hash; return true;
        case '0': flags |= flag_bits::Zero; return true;
        case '\'': flags |= flag_bits::Apostrophe; return true;
...
ParsedFormat parse_format(std::string_view fmt) {
    ...
    while (i < fmt.size()) {
        ...
        if (i + 1 < fmt.size() && fmt[i] == 'h' && fmt[i + 1] == 'h') {
            ...
            switch (fmt[i]) {
                case 'j': spec.length = LengthModifier::j; ++i; break;
                case 'z': spec.length = LengthModifier::z; ++i; break;
                case 't': spec.length = LengthModifier::t; ++i; break;
                case 'L': spec.length = LengthModifier::L; ++i; break;
                default: break;
            }
        }
